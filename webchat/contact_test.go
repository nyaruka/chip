...
func TestURNExists(t *testing.T) {
	...
	channel, err := webchat.LoadChannel(ctx, rt, twcUUID)
	require.NoError(t, err)

	exists, err := webchat.URNExists(ctx, rt, channel, "webchat:65vbbDAQCdPdEWlEhDGy4utO")
	assert.NoError(t, err)
	assert.True(t, exists)

	exists, err = webchat.URNExists(ctx, rt, channel, "webchat:123456789012345678901234")
	assert.NoError(t, err)
